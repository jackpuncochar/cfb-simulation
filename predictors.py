    ...
    def populate_sub_DMs(self):
        ...
        for unique in uniques:
            if len(self.conditions) == 2:
                ...
                self.sub_DMs[unique] = Hierarchical_DM(self.df[self.df[condition] == unique], self.conditions[1:], self.DV)
...
class Predictor_Manager():
    def __init__(self, df, formula):
        ...
        df_ = deepcopy(self.df).dropna(subset=self.variables) # drops
        df_ = df_.drop(columns=df_.columns.difference(self.variables)) # drop un-needed columns        
        
        # df_ = deepcopy(self.df).dropna(subset=[self.formula.split(' ')[0]]) # drops
        mod = smf.ols(formula=self.formula, data=df_)
        self.res = mod.fit()
    ...
    def extract_variables(self):
        target, predictors = dmatrices(self.formula, data=self.df, return_type='dataframe')
        
        return (list(target.columns) + list(predictors.columns)[1:]) # return target var, all predictors excluding intercept
